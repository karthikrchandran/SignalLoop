...
async function importContact(
  ...
  const importResponse = await request.post(
    `${apiBaseUrl}/api/v1/contacts/import`,
    {
      headers,
      multipart: {
        file: {
...
async function assignAudience(
  ...
  const response = await request.post(
    `${apiBaseUrl}/api/v1/campaigns/${campaignId}/audience`,
    {
      headers,
      data: {
...
test("live corrective smoke covers Sequences and Voice Setup UI", async ({
  page,
  request,
}) => {
  ...
  await page
    ...
    .fill(`Hello {{first_name}}, this is UI smoke ${timestamp}.`)
  ...
  const enrollResponsePromise = page.waitForResponse((response) => {
    return (
      response.url().includes("/api/v1/sequences/") &&
  ...
  const sequenceListResponse = await request.get(
    `${apiBaseUrl}/api/v1/sequences/`,
    { headers },
  )
  expect(sequenceListResponse.ok()).toBeTruthy()
  const sequenceList = (await sequenceListResponse.json()) as {
    data: SequencePublic[]
  }
  const createdSequence = sequenceList.data.find(
    (sequence) => sequence.name === `UI Smoke Sequence ${timestamp}`,
  )
  expect(createdSequence).toBeTruthy()

  const progressResponse = await request.get(
    `${apiBaseUrl}/api/v1/sequences/${createdSequence?.id}/progress`,
    { headers },
  )
  ...
  await expect(
    page.getByRole("textbox", { name: /script content/i }),
  ).toHaveValue(

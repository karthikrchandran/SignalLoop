...
test("runs prospecting research and shows outreach drafts", async ({
  page,
}) => {
    ...
    async (route) => {
            ...
            data: [
              {
                ...
                company_url: null,
                account_summary:
                  "Previous research noted chatbot pricing intent for Analytical.",
                ...
                sources: [
                  { label: "CRM contact", summary: "Ada at Analytical" },
                ],
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          account_summary:
            "Ada Lovelace is a prospect at Analytical with pricing and follow-up intent.",
          pain_points: [
          ...
          suggested_next_action:
            "Send the email draft, then call with the voice opener.",
          email_draft:
            "Subject: Idea for Analytical outreach follow-up\n\nHi Ada,",
  ...
  await expect(
    page.getByRole("heading", { name: "Lead Preparation" }),
  ).toBeVisible()
  await expect(page.getByText("Leads needing follow-up").first()).toBeVisible()
  await expect(
    page.getByRole("button", { name: "Build lead brief" }),
  ).toBeVisible()
  ...
  await expect(
    page.getByText("Previous research noted chatbot pricing intent").first(),
  ).toBeVisible()
  ...
  await expect(
    page.getByText("Ada Lovelace is a prospect at Analytical").first(),
  ).toBeVisible()
  await expect(
    page.getByText("Subject: Idea for Analytical outreach follow-up"),
  ).toBeVisible()
  await expect(
    page.getByText("Hi Ada, this is SignalLoop calling about Analytical."),
  ).toBeVisible()
  await expect(
    page.getByRole("button", { name: "Copy email draft" }),
  ).toBeVisible()
  await expect(
    page.getByRole("button", { name: "Copy voice opener" }),
  ).toBeVisible()
})

test("bulk researches selected prospects and enrolls them into outreach", async ({
  page,
}) => {
  ...
  await page.route("**/api/v1/prospecting/enroll", async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        message:
          "Added 2 prospects to Q2 Outreach and enrolled 2 in Welcome Drip.",
      }),
    })
  })
  ...
  await page
    .getByRole("checkbox", { name: "Select Ada Lovelace - Analytical" })
    .check()
  await page
    .getByRole("checkbox", { name: "Select Grace Hopper - Compiler Co" })
    .check()
